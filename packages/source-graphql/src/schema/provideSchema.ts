...
import { identity, OT, T } from '@contentlayer/utils/effect'
...
}): T.Effect<OT.HasTracer, SchemaError | utils.HashError, core.SchemaDef> =>
  T.gen(function* ($) {
    ...
    const hash = yield* $(utils.hashObject({ coreDocumentTypeDefMap, coreNestedTypeDefMap, options }))
